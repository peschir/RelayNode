...
private:
	...
	std::map<std::vector<unsigned char>, std::tuple<uint16_t, size_t, std::shared_ptr<std::vector<unsigned char> >, std::chrono::steady_clock::time_point > > txid_to_txn_map;
...
private:
	void on_disconnect() {
		...
		while (it != txid_to_txn_map.end()) {
			auto it2 = it++;
			if (std::get<2>(it2->second).unique() || !std::get<2>(it->second))
				...
				if (tx_read_pos + read_len == 32) {
					...
					if (it == txid_to_txn_map.end()) {
						txid_to_txn_map.emplace(std::vector<unsigned char>((unsigned char*)read_buff, (unsigned char*)read_buff + 32),
									std::make_tuple(obj_id, tx_pos + 1, std::shared_ptr<std::vector<unsigned char> >(), std::chrono::steady_clock::now()));
						maybe_do_send_bytes(read_buff, 32);
					} else {
						if (!std::get<2>(it->second))
							return disconnect("Hit annoying request-get-next-job race condition, resetting...");
						auto& tx_entry = job_to_tx_map[obj_id];
						assert(!tx_entry.second[tx_pos + 1]);
						tx_entry.second[tx_pos + 1] = std::get<2>(it->second);
						tx_entry.first[tx_pos + 1].size = std::get<2>(it->second)->size();
						tx_entry.first[tx_pos + 1].data = &(*std::get<2>(it->second))[0];
						provide_transaction(std::get<2>(it->second));
					}
					...
					if (tx_pos == obj_len - 1) {
						...
						while (it != txid_to_txn_map.end()) {
							auto it2 = it++;
							if (std::get<2>(it2->second).unique())
								txid_to_txn_map.erase(it2);
							else if (!std::get<2>(it2->second) && std::get<3>(it2->second) < target) {
								STAMPOUT();
								printf("%s:%u: WARNING: Discarding job %u because we did not receive all required txn in 10 seconds\n", serverHost.c_str(), (unsigned)serverPort, (unsigned)std::get<0>(it2->second));
								job_to_tx_map.erase(std::get<0>(it2->second));
								txid_to_txn_map.erase(it2);
							}
						}
						read_state = READ_NEW_MESSAGE;
			...
			case READ_TX:
			{
				...
				if (read_len > 0) {
					...
					if (read_pos == 32 + 4) {
						...
						std::get<2>(it->second) = read_obj;

						auto& tx_entry = job_to_tx_map[std::get<0>(it->second)];
						assert(!tx_entry.second[std::get<1>(it->second)]);
						tx_entry.second[std::get<1>(it->second)] = read_obj;
						tx_entry.first[std::get<1>(it->second)].size = obj_len;
						tx_entry.first[std::get<1>(it->second)].data = &(*read_obj)[0];
					}
					break;
				}
